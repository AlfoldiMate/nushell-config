...
tests/
  run.nu              the runner
  lib.nu              what a test needs: scratch, user-dir, nu-l, skip
  harness.test.nu     the harness tested with itself
  <concern>.test.nu   one file per module or concern
  fixtures/           captured output, a Ghostty config, a palette, a spec; the
...
def "test the icon is rendered" [] {
  if $nu.os-info.name != macos { skip "no rasterizer off macOS" }
  ...
}
```
...
nothing the user's config sets is present. Tests within a file share that
process; state a test wants alone goes in a `scratch` directory.
...
`skip` is a skip, reaching the end is the pass. `print` from a test goes to
...
| `skip <reason>` | stop the test, counted apart from the failures |
...
starting a shell: 332 ms. The whole suite is budgeted under 30 s so it is run
...
`skip` rather than passing vacuously; the summary counts them apart.
